...
- Test harness is non-production only and must not be registered in any runtime storage path. Later TiDB/S3 PRs may reuse invariants and test cases, but must not treat the in-memory harness as the final persistence design.
